...
namespace ECNBaxter {
    ...
    GoalResponse SetupNode::handle_goal(const GoalUUID &uuid, sptr<const PointsSetup::Goal> goal) {
    ...
    void SetupNode::savePose() {
        ...
        const std::string gripper = (sides[current_step] == Side::RIGHT) ? "right_gripper": "left_gripper";
        if (!tf_buffer.canTransform(gripper, base, tf2::TimePointZero, tf2::durationFromSec(1.0))) {
            RCLCPP_FATAL(get_logger(), "Can't find corresponding frame");
            return;
        }
        auto trans = tf_buffer.lookupTransform(base, gripper, tf2::TimePointZero, tf2::durationFromSec(1.0));
...
int main(int argc, char** argv) {
    ...
    rclcpp::spin(std::make_shared<ECNBaxter::SetupNode>(rclcpp::NodeOptions{}));
    rclcpp::shutdown();
    return 0;
}