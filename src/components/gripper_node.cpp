#include "ecn_baxter/components/gripper_node.hpp"
...
GripperNode::GripperNode(const rclcpp::NodeOptions &options)
  ...
  act_sub = create_subscription<BaxterAction>(
      ACTION_TOPIC, QOS, [this](BaxterAction::UniquePtr msg) {
  ...
  state_sub = create_subscription<EndEffectorState>(
      STATE_TOPIC, QOS,
      [this](EndEffectorState::UniquePtr msg) { this->state_process(msg); });
  cmd_pub = create_publisher<EndEffectorCommand>(CMD_TOPIC, QOS);

  vacuum_pub = create_publisher<VacuumSensor>(VACUUM_TOPIC, QOS);
}
...
void GripperNode::topics_init() {
  if (_side == "left" || _side == "right") {
    ...
    VACUUM_TOPIC = "/robot/gripper/" + _side + "/vacuum";
  }
  // If not a _side, kill the node
  else {
    RCLCPP_FATAL(
        get_logger(),
        "Parameter 'gripper_side' should be 'right' or 'left', was instead %s",
        _side.c_str());
    exit(-1);
  }
}
...
void GripperNode::action_process(const BaxterAction::UniquePtr &msg) {
  // Check if gripper properties are set and calibrated before launching any
  // action
...
void GripperNode::initialize(const EndEffectorState::UniquePtr &msg) {
  ...
  case VACUUM:
    ...
    checker = AsyncTimer([this]() { vacuum_check(); }, timer_duration);
    break;
  case CLAMP:
...
void GripperNode::parse_state() {
  ...
  if (latest_state.HasParseError()) {
    RCLCPP_FATAL(get_logger(), "%s_gripper has a JSON parsing error.",
                 _side.c_str());
  }
}
...
void GripperNode::vacuum_check() {
  ...
  while (latest_cmd != "release") {
    parse_state();
    if (latest_state.HasMember(VACUUM_SENSOR) &&
        latest_state.HasMember(VACUUM_THRESHOLD)) {
...
int main(int argc, char **argv) {
  ...
  rclcpp::spin(
      std::make_shared<ecn_baxter::GripperNode>(rclcpp::NodeOptions{}));
  rclcpp::shutdown();
  return 0;
}
