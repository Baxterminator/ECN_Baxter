...
void GamePropertiesLoader::load_game_name(const Document &d) {
  if (!d[data::GameProperties::NAME_TAG].IsString()) {
    std::cout << "[WARN] Can't find \"" << data::GameProperties::NAME_TAG << "\" tag for this game !";
    return;
  }
...
void GamePropertiesLoader::load_setup_points(const Document &d) {
  if (!d[data::GameProperties::SETUP_TAG][data::SetupRequirements::NEEDED_PTS_TAG].IsArray()) {
    std::cout << "[WARN] Can't find \"" << data::SetupRequirements::NEEDED_PTS_TAG
              << "\" tag for this game !";
    return;
  }
  // Iterate over the points to add
  const Value &pts = d[data::GameProperties::SETUP_TAG][data::SetupRequirements::NEEDED_PTS_TAG];
